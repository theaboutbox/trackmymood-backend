...
function getEvents(user, start, end) {
  return new _bluebird2.default(function (resolve, reject) {
    var params = {
      TableName: eventsTable,
      KeyConditionExpression: "user = :username and created >= :start and created <= :end",
  ...
  fields: function fields() {
    return {
      events: {
        ...
        args: {
          user: { type: new _graphql.GraphQLNonNull(_graphql.GraphQLString) },
          start: { type: new _graphql.GraphQLNonNull(_graphql.GraphQLInt) },
          end: { type: new _graphql.GraphQLNonNull(_graphql.GraphQLInt) }
        },
        resolve: function resolve(user, start, end) {
          return (0, _dynamo.getEvents)(user, start, end);
        }
      },
      posts: {
